...
class Factory
{
    ...
    protected function addServico(&$parent)
    {
        ...
        $this->addItensServico($node);

        $parent->appendChild($node);
    }

    /**
     * Includes ItensServico TAG in parent NODE
     * @param DOMNode $parent
     */

    protected function addItensServico(&$parent){

        if (!isset($this->std->servico->itensservico)) {
            return;
        }
        $pre = $this->std->servico->itensservico;

        foreach ($pre as $obj){
            $node = $this->dom->createElement('ItensServico');
            ...
            $this->dom->addChild(
                $node,
                "Quantidade",
                isset($obj->Quantidade) ? $obj->Quantidade : null,
                true
            );
            ...
            $this->dom->addChild(
                $node,
                "ValorUnitario",
                isset($obj->ValorUnitario) ? $obj->ValorUnitario : null,
                true
            );
